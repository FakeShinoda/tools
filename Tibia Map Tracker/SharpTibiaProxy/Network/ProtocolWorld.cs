...
namespace SharpTibiaProxy.Network
{
    public class ProtocolWorld
    {
            ...
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    default:
                        ...
                        Array.Copy(message.Buffer, msg.Buffer, message.Size);
                        msg.Size = message.Size;
                        client.Proxy.SendToServer(msg);
                        //throw new Exception("ProtocolWorld [ParseClientMessage]: Unprocessable packet type - 0x" + cmd.ToString("X2"));
                        break;
        ...
        private void ParseClientPingBack()
        {
            client.SendPong();
        }

        private void ParseClientPing()
        {
            client.lastPong= DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond;
            NetworkMessage msg;
        }
                ...
                while (message.ReadPosition < message.Size)
                {
                    ...
                    switch (cmd)
                    {
                        ...
                        case 0xF0:
                            ...
                            break;
                        case 0xF2:
                            ParseServerStoreBalance(message);
                            break;
                        case 0xF1:
                        ...
                        case 0xF9:
                            ...
                            break;
                        case 0x00:
                            message.ReadBytes(message.ReadPosition - packetStart);
                            break;
                        ...
                        default:
                            ...
                            throw new Exception("ProtocolWorld [ParseServerMessage]: Bypassed packet 0x" + cmd.ToString("X2"));
                    }
                }
        ...
        private void ParseServerLoginSuccess(InMessage message)
        {
            ...
            client.Map.Clear();
            Console.WriteLine("Nigga boy: " + message.Size);
            ...
            if (client.Version.Number >= ClientVersion.Version1090.Number) // From 10.80 at least
            {
                ...
                var IncCoinsTransferShop = message.ReadUShort();
            }
        }
        ...
        private void ParseServerStoreBalance(InMessage message)
        {
            var checkByte = message.ReadByte();
            if (checkByte == 1)
            {
                var randomByte = message.ReadByte();
                var randomByteTwo = message.ReadByte();
                var totalCoins = message.ReadUInt();
                var transferableCoins = message.ReadUInt();
            }
        }
        ...
        private void ParseServerPreyData(InMessage message)
        {
            var column = message.ReadByte();
            var state = message.ReadByte();
            if (state == 0)
            {
                var unlockType = message.ReadByte();
            } else if (state == 1)
            {
                // none
            } else if (state == 2)
            {
                var monsterName = message.ReadString();
                var outfit = message.ReadOutfit();
                message.ReadPosition = message.ReadPosition - 2; // we don't receive the mount id
                var bonusType = message.ReadByte();
                var bonusValue = message.ReadUShort();
                var bonusGrade = message.ReadByte();
                var timeLeft = message.ReadUShort();
            } else if (state == 3)
            {
                var listSize = message.ReadByte();
                byte index = 0;
                while (index < listSize)
                {
                    var monsterName = message.ReadString();
                    var monsterOutfit = message.ReadOutfit();
                    message.ReadPosition = message.ReadPosition - 2; // we don't receive the mount id
                    index++;
                }
            } else if (state == 4)
            {
                var preyType = message.ReadByte();
                var preyValue = message.ReadUShort();
                var preyGrade = message.ReadByte();
                var listSize = message.ReadByte();
                byte index = 0;
                while (index < listSize)
                {
            ...
            var freererollTime = message.ReadUShort();
        }
        ...
        private void ParseServerPlayerSkills(InMessage message)
        {
            ...
            for (int i = 7; i <= 12; i++)
            {
                var skillLevel = message.ReadUShort();
                var skillBase = message.ReadUShort();
            }
        }
        ...
        private void ParseServerPlayerStats(InMessage message)
        {
            ...
            var baseXp = message.ReadUShort();
            var voucherXp = message.ReadUShort();
            var lowLevelBonus = message.ReadUShort();
            var xpBoost = message.ReadUShort();
            var staminaXpBoost = message.ReadUShort();
            ...
            var xpBoostStamina = message.ReadUShort();
            var xpBoostStore = message.ReadByte();
        }
        ...
        public void SendServerPong()
        {
            if (client.LoggedIn)
            {
                var message = new OutMessage();
                message.WriteByte(0x1D);
                client.Proxy.SendToServer(message);
            }
